 ...
 *
 * Title:             Filter-Side
 * Description:       Post type archive with filters located on the side of the block.
 * Category:          filters
...
if ( $current_page && 'pagination' === $load_type ) {
...
<section <?php echo wp_kses_post( $content_block->get_block_id_attr() ); ?>class="acf-block block-filter-side<?php echo esc_attr( $content_block->get_block_classes() ); ?>" data-json-settings="<?php echo esc_attr( wp_json_encode( $json_settings ) ); ?>" data-mobile-style="<?php echo esc_attr( $mobile_style ); ?>" data-card-type="<?php echo esc_attr( $card_type ); ?>">
	<div class="container">
		<div class="block-filter-side__topbar">
			<div class="block-filter-side__selected-filter-sort-container">
				<div class="block-filter-side__selected-filters-wrapper">
					<?php if ( ! empty( $show_results_count ) ) : ?>
						<div role="alert" class="block-filter-side__result-count filter-result-count"><?php echo esc_attr( sprintf( __( '%s Results', 'catapult' ), $initial_posts->found_posts ) ); ?></div>
					<?php endif; ?>

					<div class="block-filter-side__selected-filters-container">
						<div class="block-filter-side__selected-filters selected-filters"></div>

						<button class="block-filter-side__clear-filters clear-filters" <?php echo ! $is_prefiltered ? 'disabled' : ''; ?>><?php esc_html_e( 'Clear All', 'catapult' ); ?></button>
					</div>
				</div>
		...
		<div class="block-filter-side__sidebar">
		...
		<div id="block-filter-side__posts" class="block-filter-side__posts<?php echo esc_attr( $args['post_wrapper_classes'] ); ?>">
