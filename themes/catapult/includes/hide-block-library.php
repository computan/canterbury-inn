 ...
 *   - Only users with the Administrator capability (`manage_options`) can access the `library_block` archive.
...
function hide_block_library() {
	...
	if ( is_post_type_archive( 'library_block' ) && ! current_user_can( 'manage_options' ) ) {
		global $wp_query;
		$wp_query->set_404();
		status_header( 404 );
		nocache_headers();
